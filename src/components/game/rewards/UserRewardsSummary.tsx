...
interface UserRewardsSummaryProps {
  rewards: UserRewardDetailed[];
}

export function UserRewardsSummary({ rewards }: UserRewardsSummaryProps) {
  const total = rewards.length;
  const available = rewards.filter((r) => r.rewardStatus === "available").length;
  const claimed = rewards.filter((r) => r.rewardStatus === "claimed").length;
  const pending = rewards.filter((r) => r.rewardStatus === "pending").length;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-sky-400/60 bg-linear-to-br from-sky-500/15 via-sky-400/10 to-sky-300/20 px-4 py-3 shadow-sm">
        <div className="pointer-events-none absolute -left-3 -top-3 h-12 w-12 rounded-full bg-sky-300/30 blur-xl" />
      ...
      <div className="relative overflow-hidden rounded-2xl border border-amber-400/60 bg-linear-to-br from-amber-500/15 via-amber-400/10 to-amber-300/20 px-4 py-3 shadow-sm">
        <div className="pointer-events-none absolute -right-4 top-2 h-10 w-10 rounded-full bg-amber-300/40 blur-xl" />
