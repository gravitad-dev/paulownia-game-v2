...
export function RetroBackground() {
  ...
  const mouseRef = useRef({ x: -1000, y: -1000, vx: 0, vy: 0, lastX: -1000, lastY: -1000, lastTime: 0 });
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      for (let i = 0; i < physicsRef.current.length; i++) {
        ...
        const swayForce = Math.sin(time * leaf.swaySpeed + leaf.swayPhase) * 0.02;
        leaf.vx += swayForce;
  ...
  return (
    <div 
      className="fixed inset-0 z-[-1] overflow-hidden bg-[#050a1f] pointer-events-none font-mono"
    >
      {/* Stars */}
      ...
      {leaves.map((leaf, index) => (
        ...
        >
             ...
             <div className="w-10 h-10 relative opacity-90">
              {/* Stem */}
              <div className={`absolute -top-1 left-4 w-2 h-3 ${leaf.color} brightness-50`} />
              {/* Top Bumps */}
